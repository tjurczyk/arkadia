...
local weapons = { "darda", "dardy", "multon", "kord", "puginal", "gladius", "topor", "berdysz", "siekier", "czekan",
...
local torso = { "brygantyn", "napiersnik", "kirys", "kolczug", "karacen", "kaftan", "tunik", "zbroj", "bajdan[ay]",
    "anim[eay]", "kozus", "kurt", "kamizel", "becht", "pancerz", "zbro. plytow", "polpancerz", "nabrzusznik", "bajdan",
    "aketo" }
local head = { "helm", "burgonet", "misiurk", "kaptur", "morion", "basinet", "salad", "przylbic", "diadem", "szyszak",
    "narbut[ay]", "armet", "casquett", "czapk", "beret", "turban", "gigantyczn. wzmacnian. czaszk", "barbut", "kapalin",
    "koron[^k]", "klobuk" }
local legs = { "nagolennik", "spoden", "nogawic", "buty", "butow", "trzewik", "spodni", "spodnic", "naudziak", "sandal",
    "nakolannik", "nabiodr", "pantofel", "muszkieter" }
local hands = { "nareczak", "naramiennik", "rekawic", "karwasz" }
local wear = { "futro", "kubraczek", "koszul", "sukni", "sukien", "plaszcz", "peleryn", "tog", "szat",
...
local jewelery = { "pierscien(?!iowa)", "naszyjnik", "bransolet", "spink", "talizman", "amulet", "kolczyk", "lancuszki",
    "koral", "wisior", "medalion", "lancusz", "brosz", "szarf", "koli[iae]", "sygnet", "obracze?k", "potrojn. sznur.+",
    "cwiek( |$|i|ow)(?!ana)", "serduszk", "grzebyk" }
local gems = { "obsydia(ny|now|n)", "labrado(ry|row|r)", "oliwi(ny|now|n)", "gaga(ty|tow|t)", "fluory(ty|tow|t)",
...
local preferred_magics = function(name)
    ...
    return key and
        (not table.is_empty(table.n_intersection(properties.type, preferred_types)) or table.contains(additional_magics, key))
end

scripts.inv.pretty_containers.name_transformers = {
    ["magic"] = {
        check = function(name) return table.contains(scripts.inv.magics_data.magics, string.lower(name)) end,
        transform = function(
            name)
            return "<" .. scripts.inv.magics_color .. ">" .. name
        end
    },
    ["preferred_magic"] = {
        check = preferred_magics,
        transform = function(name)
            return string.format(
                "<spring_green>***<reset> <%s>%s <spring_green>***", scripts.inv.magics_color, name)
        end
    },
    ["keys"] = {
        check = function(name) return table.contains(scripts.inv.magic_keys_data.magic_keys, string.lower(name)) end,
        transform = function(
            name)
