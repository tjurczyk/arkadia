...
function scripts.ui.no_weapon_alert:init()
    self.handler_wield = scripts.event_register:register_singleton_event_handler(self.handler_wield, "weapon_state", function(_, state)
        if state then
            self.is_knocked_off = false
    ...
    self.handler_weapon_broken = scripts.event_register:register_singleton_event_handler(self.handler_weapon_broken, "weaponBroken", function() self.is_broken = true end)
end

function scripts.ui.no_weapon_alert:show_alert()
    if self.can_dispatch_alert and not self.is_knocked_off and not self.is_broken then
