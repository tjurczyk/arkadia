...
```
lua local a="master"local b="https://codeload.github.com/tjurczyk/arkadia-skrypty/zip/"..a local c=getMudletHomeDir().."/scripts.zip"local d=getMudletHomeDir().."/arkadia-"..a.."/"local e=getMudletHomeDir().."/arkadia/"function installScripts()pcall(deleteDir,e)registerAnonymousEventHandler("sysDownloadDone","handleDownload",true)downloadFile(c,b)cecho("\n<CadetBlue>(skrypty)<tomato>: Pobieram aktualna paczke skryptow\n")end function handleDownload(f,g)if g~=c then return true end registerAnonymousEventHandler("sysUnzipDone","handleUnzipEvents",true)registerAnonymousEventHandler("sysUnzipError","handleUnzipEvents",true)unzipAsync(c,getMudletHomeDir())end function handleUnzipEvents(h,...)if h=="sysUnzipDone"then cecho("\n<CadetBlue>(skrypty)<tomato>: Skrypty rozpakowane\n")os.remove(c)os.rename(d,e)installPackage(e.."Arkadia.xml")uninstallPackage("generic_mapper")uninstallPackage("skrypty3_master")elseif h=="sysUnzipError"then cecho("\n<CadetBlue>(skrypty)<tomato>: Blad podczas rozpakowywania skryptow\n")end end function deleteDir(i)for j in lfs.dir(i)do local k=i..'/'..j if j~="."and j~=".."then if lfs.attributes(k,'mode')=='file'then os.remove(k)elseif lfs.attributes(k,'mode')=='directory'then deleteDir(k)end end end lfs.rmdir(i)end installScripts()
```
